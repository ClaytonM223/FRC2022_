...
public class GyroCorrect extends CommandBase {
   ...
   */
  public GyroCorrect(double angle , double speed) {
    m_speed = speed;
    m_angle = angle;
  ...
  public void execute() {
    ...
    if (Math.abs(m_angle) > Math.abs(error)+1){
      RobotContainer.driveTrain.drive.arcadeDrive(0, m_speed);
    } else if(Math.abs(m_angle) < Math.abs(error)-1){
      RobotContainer.driveTrain.drive.arcadeDrive(0, -m_speed);
    } else{
  ...
  public boolean isFinished() {
    if(Math.abs(m_angle) > Math.abs(pigeon.getYaw()-1)){
      return false;
